...
def clean_csv(df):
    ...
    df = df[-df.isin(['Nom', 'ETABLISSEMENT'])['ETABLISSEMENT_Nom']] # delete raw were the name of the school misses
    df = df.applymap(text_prepare)  # normlisation of writting
    df.dropna(how='all', axis=1)    # delete empty raws
    df = df[-df.isna().all(axis=1)]

    return df


def merge_csv(csv_dir, merged_csv_path, header):
    list_name = os.listdir(csv_dir)
    list_df = []
    ...
    df.to_csv(merged_csv_path, sep=';', index=False)
...
if __name__ == '__main__':
    ...
    merge_csv(csv_dir='./data/csv/', merged_csv_path = './data/merged.csv', header=header)
